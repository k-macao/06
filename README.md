...
2. **确认真实频道**：优先使用格式有效的 `channel_id`；缺失时仅允许从真实 YouTube 频道页解析。搜索结果页和非 YouTube 占位链接会被拒绝。
3. **官方 RSS 唯一内容源**：从 `https://www.youtube.com/feeds/videos.xml?channel_id=UCxxx` 读取真实标题、视频链接、摘要与发布时间。
4. **90 天新鲜度门槛**：最新有效 RSS 条目在 `ACTIVE_THRESHOLD_DAYS` 内才可进入日报。
...
`enrich_with_verified_content()` 只补充展示字段，不改写来源内容：

- `title` 必须与 `original_title` 完全一致；
- `link` 必须是 YouTube 视频、Shorts 或直播链接；
- `published` 必须来自 RSS；
- 每条内容带 `source: youtube_rss`、`source_channel_id` 与 `is_mock: false`；
- RSS 有几条有效内容就展示几条（最多 3 条），绝不为了固定版面补足模拟条目。

旧版 `MOCK_TITLES_POOL`、模拟日期和 `?v=mock` 链接已从生产抓取器中移除。
...
- **内容层**：`is_mock: true`、`?v=mock`、标题与 `original_title` 不一致、来源频道不一致、缺失/未来日期均为 FAIL。
...
> 📌 2026-08-15 首次全量排查发现 237 条内容中 **219 条（92.4%）为 mock 伪链接**。2026-08-16 起生产抓取链已移除所有 Mock 兜底；旧报告也已清除伪链接与改写标题。目录中暂未核实的记录保留作后续修订，但不会进入日报。
