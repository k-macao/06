...
- 报告内的模拟标记、伪链接、改写标题和严重失实来源均为 FAIL；
    ...
    for index, item in enumerate(items or [], start=1):
        ...
        if not CHANNEL_ID_RE.fullmatch(source_channel_id):
            issues.append({
                "level": "FAIL",
                "code": "PROVENANCE",
                "msg": f"条目{index} source_channel_id 缺失或格式无效: {source_channel_id or '空'}",
            })
...
def run_audit(
    ...
    for kol in kols:
        ...
        results.append({
            ...
            "pool_titles": altered_count,  # 兼容旧版 JSON 消费方
